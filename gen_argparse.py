"""
gen_argparse.py
Update: 2020-10-19 include pi_audio for Process_count_audio.py
...
# used only for audio processing
parser.add_argument('-pi_audio', '--pi_audio', default='yes', type=str, help='does pi audio exist? default is yes, else type no')

# used only for env processing
parser.add_argument('-pi_env', '--pi_env', default='yes', type=str, help='does pi env exist? default is yes, else type no')
...
pi_audio = True if args.pi_audio == 'yes' else False
pi_env = True if args.pi_env == 'yes' else False
print(f'pi audio: {pi_audio}, pi env: {pi_env}')
